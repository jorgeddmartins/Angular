...
function renderCalendar(startDay, totalDays, currentDate) {
    ...
    for(; i <= totalDays; i++) {
        ...
        // Desable All Days & Weeks Before Today in the Current Month
        ...
        // Removes Class .today from all MOnths but the current Month
	    ...
	    // Click Event Logic for next-month & prev-month
        $.fn.checkAllMonth = function() {
        	return this.on(ck, '*', function() {

        		let claSs = $(this)[0].className;

        		// Next-month Click Event Enables all days & weeks for monthes ahead
        		if(claSs.indexOf('next-month') != -1) {
        		...
        		// Prev-month Click Event Desables all Days & Weeks for Previous Monthes
        		else if(claSs.indexOf('prev-month') != -1) {
...
function clearCalendar() {
    let $trs = $('.TeleSalesGridCalendar .table tr').not(':eq(0)');
    $trs.remove();
    $('.month-year').empty();
}
...
$(function() {
	...
	$.fn.GridDefaultCalendar = function(CalendarOPtions) {
		...
		let DefaultOptions = {
			...
			OptCalend: 'datetimepicker' + nbigCal,

			DateTPick: 'datetimepicker' + nsmallCal,

			AvailableCal: 'AvailableFrom',
		...
		let settingsCal = $.extend( true, {
			...
			dateTpop: '<div class="dateTimepopUp">' +
							...
							'<span class="manuallyQueueLimit" id="manuallyQueueLimit">105</span>' +
							'<span class="rotationLimit" id="rotationLimit">107</span>' +
						'</div>',
		...
		$.extend( RunCalen.prototype, {
			...
			calendarType: function() {
				...
				// $('.' + settingsCal.calenDrop).draggable();
				...
				$.fn.LeadCalen = function() {
					return this.bind(drag, function() {
						...
						// Hidden Input Left Value
						$('#' + settingsCal.avaiformHidden).val(offsetLeft);

						// Hidden Input Value Each function 
				...
				$.fn.RunTeleGridcalendar = function() {
				    ...
				    $.fn.GridGlTele = function() {
				    	return this.on(ck, '*', function() {
				    		...
				    		// Runs Next Month
				    		const nextMonth = fClass.indexOf('next-month') != -1 ? navigationHandler(1) && $('.dyItem').removeClass('PrevDays') : '';
				    	});
				    }
				    doc.GridGlTele();
				    ...
				    $.ajax({
				    	type: 'GET',
				    	cache: false,
				    	url: 'assets/js/GridTeleSalesCalendar/GridTeleSalesCalendar.json',
				    	datatype: 'json',
					...
					$.fn.OpenCalendGl = function() {
						return this.on(ck, '*', function() {
							...
							if(id == settingsCal.OptCalend) { telSHourPickerCal('105%', '-122%'); }

							else if(id == settingsCal.DateTPick) { datetimepicker2Cal('-12%', '-69%'); }
					...
					$.fn.datePickerCl = function() {
						...
						return this.on(ck, '.datepicker', function() {

							$('.datepicker').removeClass('valTh');
							$(this).addClass('valTh');
						});
					}
					doc.datePickerCl();
					...
					function telSHourPickerCal(tops, lefts) {

						$(settingsCal.CalGrid + 0).show().css({ top: tops, left: lefts });

						$('.TeleGridSecd, .TeleGridHour').show();
						$('.TeleGridHour, .TeleGridDays').css({ 'margin-right': '7px' });
						...
						// Days PopUp Template Injection
						$(settingsCal.dateTpop).appendTo('.CalenFloor .table');
					...
					function datetimepicker2Cal(tops, lefts) {
					...
					function CalenDragg() {
						...
						$.fn.DragTelCal = function(dataHideH, bodyTop) {
							...
							return this.bind(drag, function() {
								...
								// Top Value displays in Hidden Input
								dataHideH.val(offTop);

								// Each Function for hidden inputs
						...
						$(settingsCal.CalGrid + 0).DragTelCal($('#hidePickHour'), 250.140625); // 535.140625
					}
					...
					$.fn.DatePickerVal = function() {
						return this.on(ck, '.DatePickerVal .dyItem, .DatePickerVal .hourItem', function() {
					...
					$.fn.dyItemAction = function() {
				    	return this.on(ck, '.dyItem', function() {
				    		...
				    		if($('.hourItem').hasClass('addHourValue') && !$('.secondItem').hasClass('addMinuteValue')) {
				    			...
				    			$(dateHourPick).val(NullHeader($(this).html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + nowHour + ':' + $('.addMinuteValue').html());

				    		} else if($('.hourItem').hasClass('addHourValue') && $('.secondItem').hasClass('addMinuteValue')) {

				    			$(dateHourPick).val(NullHeader($(this).html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $('.addHourValue').html().slice(0, -2) + $('.addMinuteValue').html());
				    		}

							$('.birthForm .valTh').val(NullHeader($(this).html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html());

				   			MonthTransf();

				    		$('.dateTimepopUp').show();

				    		$('.dateDay').html(NullHeader($(this).html()));

				    		$('.dateYear').html($('.ThisYearTop').html());

				    		$('.dateMonth').html($('.ThisMonthTop').html());
							...
							if($('#manuallyQueueLimit').html() >= $('#rotationLimit').html()) { RotationLimit($(this));
					...
					$.fn.dyItemActionSmall = function() {
						return this.on('click', '.smallCalend .dyItem', function() {
							...
							if($('.hourItem').hasClass('addHourValue') && !$('.secondItem').hasClass('addMinuteValue')) {
								...
								$('#datetimepicker2').val(NullHeader($(this).html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + '00');
								...
								$('#datetimepicker2').val(NullHeader($(this).html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $('.addHourValue').html().slice(0, -2) + '00');
							}
				    ...
				    $.fn.hourItemAction = function(hItm) {
				    	return this.on(ck, '.hourItem', function() {
				    ...
				    $.fn.hourItemAction = function(hItm) {
						return this.on('click', '.bigCalend .hourItem', function() {
							...
							if(!$('.dyItem').hasClass('getday BThDay') && $('.secondItem').hasClass('addMinuteValue')) {

								$('#datetimepicker3').val(getDay + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $(this).html().slice(0, -2) + $('.addMinuteValue').html());
								
							} else if($('.dyItem').hasClass('getday BThDay') && !$('.secondItem').hasClass('addMinuteValue')) {

								$('#datetimepicker3').val(NullHeader($('.getday').html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $(this).html());
								...
								$('#datetimepicker3').val(NullHeader($('.getday').html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $(this).html().slice(0, -2) + $('.addMinuteValue').html());
							}
							MonthTransf();
					...
					$.fn.hourItemAction2 = function(hItm) {
						return this.on('click', '.smallCalend .hourItem', function() {
							...
							if(!$('.dyItem').hasClass('getday SThDay') && $('.secondItem').hasClass('addMinuteValue')) {

								$('#datetimepicker2').val(getDay + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $(this).html());

							} else if($('.dyItem').hasClass('getday SThDay') && !$('.secondItem').hasClass('addMinuteValue')) {

								$('#datetimepicker2').val(NullHeader($('.getday').html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $(this).html());
							} 
				    ...
				    $.fn.minuteItemAction = function() {
				    	return this.on(ck, '.secondItem', function() {

				    		$('.secondItem').removeClass('addMinuteValue');
				    		$(this).addClass('addMinuteValue');

				    		if(!$('.hourItem').hasClass('addHourValue') && !$('.dyItem').hasClass('getday')) {

				    			$(dateHourPick).val(getDay + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + nowHour + ':' + $(this).html());

				    		} else if($('.hourItem').hasClass('addHourValue') && !$('.dyItem').hasClass('getday')) {

				    			$(dateHourPick).val(getDay + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $('.addHourValue').html().slice(0, -2) + $(this).html());

				    		} else if($('.hourItem').hasClass('addHourValue') && $('.dyItem').hasClass('getday')) {

				    			$(dateHourPick).val(NullHeader($('.getday').html()) + '/' + $('.ThisMonthTop').html() + '/' + $('.ThisYearTop').html() + ' ' + $('.addHourValue').html().slice(0, -2) + $(this).html());
				    		}
				    		MonthTransf();

				    	});
				    }
				    doc.minuteItemAction();
				...
				$.fn.MonthTop = function() {
					return this.on(ck, '.ThisMonthTop', function() {
				...
				$.fn.dropMonth = function() {
					return this.on(ck, '*', function() {
						...
						// Start Months Floor
						// let TMTop = findClass.indexOf('ThisMonthTop') != -1 ? $('.ThisMonthTopfloor').slideToggle(100) && $('.ThisYearTopfloor').hide() : '';
						...
						switch(Macth) {
							...
							case 'May':
								...
								else if($(this).hasClass('novDrop')) { overCalendar(6) }	
								else if($(this).hasClass('decDrop')) { overCalendar(7) }
						...
						if($(this).hasClass('ThisYearTop')) {
							...
							$.ajax({
						    	...
						    	success: function(gridtelecalendataM) { doc.InjeYearM(gridtelecalendataM); }
						    });
							...
							$.fn.InjeYearM = function(gridtelecalendataM) {
								...
								// if($('.ThisYearTop').html() >= 2010 && $('.ThisYearTop').html() <= 2019) { 

								// 	$('.yr-2010').addClass('selectedYM') 
								// }
				...
				doc.on('DOMSubtreeModified', '.ThisYearTop', function() { trPopUps(); });

				// Testing Years Length Between two Years
				var years = getYears('1980-5-1', '2000-4-6');
				...
				function getYears(from, to) {
				    ...
				    for (var i = d1.getFullYear(); i <= d2.getFullYear(); i++) { yr.push(i); }
				...
				// Removes the HTML class click event( to close calendar )
				doc.on('mouseover', '#datetimepicker3, #datetimepicker2', function() { $('html').removeClass('closeCalend'); }); 
